...
type Body = NitroFetchOptions<string>['body']
...
class Service {
	...
	async post<T>(url: string, body: Body, options = {}) {
		return (await customFetch(url, { method: 'POST', body, ...options })) as T
	}

	async put<T>(url: string, body: Body, options = {}) {
		return (await customFetch(url, { method: 'PUT', body, ...options })) as T
	}

	async patch<T>(url: string, body: Body, options = {}) {
